#include "aes/primitives.h"
#include <catch.hpp>

using aes::matrix;

TEST_CASE( "AES S-box", "[aes]" ) {
    CHECK( aes::s_box[0x53] == 0xed );
}

TEST_CASE( "AES S-box inverse", "[aes]" ) {
    for( int i = 0; i < 256; i++ ) {
        INFO( "i = " << i );
        CHECK( aes::s_box_inv[aes::s_box[i]] == i );
        CHECK( aes::s_box[aes::s_box_inv[i]] == i );
    }
}

TEST_CASE( "AES addroundkey", "[aes]" ) {
    matrix data{
        0xBA, 0x84, 0xE8, 0x1B,
        0x75, 0xA4, 0x8D, 0x40,
        0xF4, 0x8D, 0x06, 0x7D,
        0x7A, 0x32, 0x0E, 0x5D,
    };
    matrix key{
        0xE2, 0x91, 0xB1, 0xD6,
        0x32, 0x12, 0x59, 0x79,
        0xFC, 0x91, 0xE4, 0xA2,
        0xF1, 0x88, 0xE6, 0x93,
    };
    matrix ans{
        0x58, 0x15, 0x59, 0xCD,
        0x47, 0xB6, 0xD4, 0x39,
        0x08, 0x1C, 0xE2, 0xDF,
        0x8B, 0xBA, 0xE8, 0xCE,
    };
    CHECK( aes::addroundkey(data, key) == ans );
    CHECK( aes::addroundkey_inv( aes::mixcolumn_inv(data), key )
            == aes::mixcolumn_inv( aes::addroundkey(data, key) )
         );
}

TEST_CASE( "AES subbytes", "[aes]" ) {
    matrix m{
        0x00, 0x3C, 0x6E, 0x47,
        0x1F, 0x4E, 0x22, 0x74,
        0x0E, 0x08, 0x1B, 0x31,
        0x54, 0x59, 0x0B, 0x1A,
    };
    matrix ans{
        0x63, 0xEB, 0x9F, 0xA0,
        0xC0, 0x2F, 0x93, 0x92,
        0xAB, 0x30, 0xAF, 0xC7,
        0x20, 0xCB, 0x2B, 0xA2,
    };
    CHECK( aes::subbytes(m) == ans );
    CHECK( aes::subbytes_inv(ans) == m );
}

TEST_CASE( "AES shiftrow", "[aes]" ) {
    matrix m{
        0x63, 0xEB, 0x9F, 0xA0,
        0xC0, 0x2F, 0x93, 0x92,
        0xAB, 0x30, 0xAF, 0xC7,
        0x20, 0xCB, 0x2B, 0xA2,
    };
    matrix ans{
        0x63, 0xEB, 0x9F, 0xA0,
        0x2F, 0x93, 0x92, 0xC0,
        0xAF, 0xC7, 0xAB, 0x30,
        0xA2, 0x20, 0xCB, 0x2B,
    };
    CHECK( aes::shiftrow(m) == ans );
    CHECK( aes::shiftrow_inv(ans) == m );
}

TEST_CASE( "AES mixcolumn", "[aes]" ) {
    matrix m{
        0x63, 0xEB, 0x9F, 0xA0,
        0x2F, 0x93, 0x92, 0xC0,
        0xAF, 0xC7, 0xAB, 0x30,
        0xA2, 0x20, 0xCB, 0x2B,
    };
    matrix ans{
        0xBA, 0x84, 0xE8, 0x1B,
        0x75, 0xA4, 0x8D, 0x40,
        0xF4, 0x8D, 0x06, 0x7D,
        0x7A, 0x32, 0x0E, 0x5D,
    };
    CHECK( aes::mixcolumn(m) == ans );
    CHECK( aes::mixcolumn_inv(ans) == m );
}
