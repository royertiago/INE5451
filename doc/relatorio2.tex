...
Por último, temos que gerar e testar todas as combinações de chaves.
Para isso, usamos uma matriz de índices.
\begin{cppcode}
        std::vector<aes::polynomial>::iterator index[4][4];
        for( int i = 0; i < 4; i++ )
        for( int j = 0; j < 4; j++ ) {
            index[i][j] = voted[i][j].begin();
        }
\end{cppcode}
\verb"*index[i][j]" é o valor do byte $ij$ para a chave atual.

O restante do código meramente itera sobre o espaço de chaves.
\begin{cppcode}
...
A função \verb"test_key" testa se a chave passada
corretamente decifra os valores da integral.
Como em nosso formato de entrada nós não disponibilizamos o texto claro,
esta função executa o algoritmo de decriptação do AES
em dois valores diferentes da integral.
Caso o resultado difira em, exatamente, uma posição,
consideramos aquela chave válida.

Observe que este trecho do código que testa todas as combinações possíveis
faz com que, teoricamente,
apenas uma integral seja suficiente para que o programa quebre a chave.
